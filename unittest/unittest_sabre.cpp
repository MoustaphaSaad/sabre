...
TEST_CASE("[sabre]: spirv")
{
	mn_defer{mn::memory::tmp()->clear_all();};

	auto base_dir = mn::path_join(mn::str_tmp(), DATA_DIR, "spirv");
	auto files = mn::path_entries(base_dir, mn::memory::tmp());
	for (auto f: files)
	{
		if (f.kind != mn::Path_Entry::KIND_FILE || f.name == "." || f.name == "..")
		...
		if (mn::path_is_file(mn::str_tmpf("{}.out", filepath)) == false)
		{
			mn::log_warning("missing spirv output for '{}'", filepath);
			continue;
		}
		...
		auto [answer_, err] = sabre::spirv_gen_from_file(filepath, f.name, mn::str_lit(""), {});
		CHECK(err == false);
		auto answer = answer_;
		mn_defer{mn::str_free(answer);};
		mn::str_replace(answer, "\r\n", "\n");
